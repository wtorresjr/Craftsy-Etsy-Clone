      ...
      <div className="cartItemCard" key={item.id}>
          ...
          <div className="cartItemImgContainer">
            <NavLink to={`/products/${item.product_id}`}>
              <img
                ...
                src={item.preview_image_url && item.preview_image_url[0]}
              />
            </NavLink>
          </div>
              ...
              >
                {productsArr?.map((product) => {
                  if (product.id === item.product_id) {
                    return [...Array(product.quantity).keys()].map((i) => (
        ...
        <div className="cardFooter">
          <div className="personalizeItemContainer">
            {/* <button onClick={handleNonFunctioningLinks}>
              <i className="fa-solid fa-tag"></i>Apply shop coupon codes
            </button>
            <button onClick={handleNonFunctioningLinks}>
              <i className="fa-solid fa-plus"></i>Add a note to{" "}
              {productInfoObj[item.id]?.Seller?.first_name}{" "}
              {productInfoObj[item.id]?.Seller?.last_name}
            </button>
            <label onClick={(e) => handleNonFunctioningLinks(e)}>
              <input type="checkbox" />
            ...
            <p>
              ...
              {shippingPrice > 10.0 ? (
                <span style={{ color: "red" }}>
                  ${(shippingPrice / cartItemsArray.length).toFixed(2)}
                </span>
              ) : (
